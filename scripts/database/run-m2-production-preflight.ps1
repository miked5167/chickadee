...
$BackupName = "advisor-directory-production-$Timestamp"
...
$CliWorkDirectory = Join-Path $BackupDirectory 'm2-cli-work'
...
function Invoke-SupabaseCli([string[]]$CliArguments, [string]$FailureMessage) {
  ...
  try {
    # Windows PowerShell wraps native stderr as ErrorRecord objects. Supabase CLI
    # writes informational workdir messages there even when it exits successfully.
...
if ($BackupRootResolved.StartsWith($RepositoryResolved, [StringComparison]::OrdinalIgnoreCase)) {
  throw 'Protected backup root must be outside the repository.'
}
...
try {
  ...
  $WorktreeBefore = @(git status --porcelain=v1)
  $WorktreeBeforeText = $WorktreeBefore -join "`n"
  ...
  if ((Get-FileHash -Algorithm SHA256 -LiteralPath $M2Path).Hash.ToLowerInvariant() -ne $M2Sha256) { throw 'M2 hash differs from the reviewed hash.' }
  ...
  $Identity = Invoke-ReadOnlyScalar @"
...
"@
  ...
  if ($IdentityObject.database -ne $DatabaseName -or $IdentityObject.connected_user -ne 'postgres') {
    throw 'Routed production database identity is ambiguous.'
  }

  $History = (Invoke-ReadOnlyScalar "SELECT coalesce(json_agg(json_build_object('version', version, 'name', name) ORDER BY version)::text, '[]') FROM supabase_migrations.schema_migrations") | ConvertFrom-Json
  if (@($History).Count -ne 1 -or $History.version -ne $M1Version -or $History.name -ne $M1Name) {
    throw 'Production migration history differs from exact M1-only state.'
  }
...
"@
  if ($CompaniesShapeMatches -ne 't') { throw 'Production companies/updated_at prerequisite differs.' }

  $FunctionObservation = (Invoke-ReadOnlyScalar @"
SELECT json_build_object(
  ...
  'body_length', pg_catalog.length(pg_catalog.btrim(p.prosrc, E' \t\r\n')),
  'body_md5', pg_catalog.md5(pg_catalog.btrim(p.prosrc, E' \t\r\n'))
)
...
"@) | ConvertFrom-Json
  ...
  $FunctionMismatches = @($FunctionMatchProperties | Where-Object { $FunctionObservation.$_ -ne $true })
  if ($FunctionMismatches.Count -ne 0) {
    $SafeFunctionDiagnostic = [ordered]@{
      mismatches = $FunctionMismatches
      body_length = $FunctionObservation.body_length
      body_md5 = $FunctionObservation.body_md5
    } | ConvertTo-Json -Compress
    throw "Production timestamp function differs from the adopted definition or attributes. Safe diagnostic: $SafeFunctionDiagnostic"
  }

  $CompaniesTimestampTriggerCount = [int](Invoke-ReadOnlyScalar @"
SELECT count(*)
...
"@)
  if ($CompaniesTimestampTriggerCount -ne 0) { throw 'An intended or equivalent companies timestamp trigger already exists.' }
...
"@) | ConvertFrom-Json
  foreach ($Name in $ExpectedCounts.Keys) {
    if ([int64]$CountsBefore.$Name -ne $ExpectedCounts[$Name]) { throw "Production count differs for public.$Name." }
  }

  New-Item -ItemType Directory -Path $BackupDirectory | Out-Null
  [pscustomobject]@{
    database_name = $IdentityObject.database
    connected_user = $IdentityObject.connected_user
    server_version = $IdentityObject.server_version
    captured_at_utc = (Get-Date).ToUniversalTime().ToString('o')
    expected_project_ref = $ProjectRef
    pooler_route = $PoolerHost
    identity_match = $true
...
"@
  ...
  $CountsAfter = (Invoke-ReadOnlyScalar @"
SELECT json_build_object(
  'companies', (SELECT count(*) FROM public.companies),
  'advisors', (SELECT count(*) FROM public.advisors),
  'listing_claims', (SELECT count(*) FROM public.listing_claims),
...
"@) | ConvertFrom-Json
  foreach ($Name in $ExpectedCounts.Keys) {
    if ([int64]$CountsAfter.$Name -ne $ExpectedCounts[$Name] -or [int64]$CountsAfter.$Name -ne [int64]$CountsBefore.$Name) {
      throw "Production count changed during preflight for public.$Name."
    }
  }
  ...
  Copy-Item -LiteralPath $M1Path,$M2Path -Destination (Join-Path $CliWorkDirectory 'supabase\migrations')
  ...
  ) 'Pinned Supabase CLI dry run failed.'
  ...
  if ($DryRunMigrations.Count -ne 1 -or $DryRunMigrations[0] -ne $M2Filename) {
    throw 'Pinned CLI dry run does not contain exactly the reviewed M2 migration.'
  }
  ...
  $WorktreeAfterText = @(git status --porcelain=v1) -join "`n"
  if ($StagedAfter.Count -ne 0) { throw 'Preflight unexpectedly staged repository files.' }
  if ($WorktreeAfterText -ne $WorktreeBeforeText) { throw 'Repository worktree changed during production preflight.' }
  ...
  if ($ArchiveEntries -ne 1321) { throw "Protected archive has $ArchiveEntries entries instead of exact M1 count 1321." }

  $PreflightSummary = [ordered]@{
    status = 'success'
    captured_at_utc = (Get-Date).ToUniversalTime().ToString('o')
    project_ref = $ProjectRef
    routed_identity_match = $true
    migration_history = @(@{ version = $M1Version; name = $M1Name })
    intended_or_equivalent_companies_trigger_count = 0
    timestamp_function_matches_adopted_definition = $true
    application_counts = $ExpectedCounts
    m1_sha256 = $M1Sha256
    m2_sha256 = $M2Sha256
    ...
    archive_list_validation = 'passed'
    schema_stream_validation = 'passed'
    data_stream_validation = 'passed'
    site_smoke_status = 200
    api_total = 202
    production_changes_made = $false
    baseline_ddl_executed = $false
  }
  $PreflightSummary | ConvertTo-Json -Depth 6 | Set-Content -Encoding utf8 -LiteralPath (Join-Path $BackupDirectory 'preflight-summary.json')
  @{ success = $true; status = 'success'; production_changes_made = $false; baseline_ddl_executed = $false } |
    ConvertTo-Json | Set-Content -Encoding utf8 -LiteralPath (Join-Path $BackupDirectory 'status.json')
  $StatusWritten = $true

  Write-EvidenceChecksumManifest

  & node scripts/database/validate-production-evidence.mjs $BackupDirectory --target baseline
  Assert-LastExitCode 'Fresh protected M1 evidence validation failed.'
  & node scripts/database/validate-migrations.mjs
  Assert-LastExitCode 'Final repository validation failed.'

  Write-Output "M2 production preflight passed with no production mutation. Evidence package: $BackupName"
  Write-Output "History: ${M1Version}_${M1Name} only; trigger/equivalent count: 0; counts: 202/177/0/0/0."
  Write-Output "Pinned CLI dry run: $M2Filename only."
} catch {
  $Failure = $_
  if (Test-Path -LiteralPath $BackupDirectory) {
    try {
      $PreflightSummaryPath = Join-Path $BackupDirectory 'preflight-summary.json'
      if (Test-Path -LiteralPath $PreflightSummaryPath) {
        $FailedSummary = Get-Content -Raw -LiteralPath $PreflightSummaryPath | ConvertFrom-Json
        ...
        $FailedSummary | ConvertTo-Json -Depth 6 | Set-Content -Encoding utf8 -LiteralPath $PreflightSummaryPath
      }
      @{ success = $false; status = 'failed'; production_changes_made = $false; baseline_ddl_executed = $false; error = $Failure.Exception.Message } |
        ConvertTo-Json | Set-Content -Encoding utf8 -LiteralPath (Join-Path $BackupDirectory 'status.json')
      Write-EvidenceChecksumManifest
    } catch {
      Write-Warning 'Could not finalize the failed evidence-package status/checksums.'
    }
  }
  throw $Failure
} finally {
