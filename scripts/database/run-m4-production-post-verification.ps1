...
$ExpectedHistorySignature = "${M1Version}:${M1Name},${M2Version}:${M2Name},${M3Version}:${M3Name},${M4Version}:${M4Name}"
...
$PostName = "advisor-directory-production-post-m4-verification-$Timestamp"
...
function Assert-ReadOnlyQueryFails([string]$Sql, [string]$FailureMessage) {
  $PreviousErrorActionPreference = $ErrorActionPreference
  try {
    $ErrorActionPreference = 'Continue'
    & $Psql @script:PsqlArguments -Atq --command "BEGIN READ ONLY; $Sql; ROLLBACK;" 1>$null 2>$null
...
$BackupRootResolved = [IO.Path]::GetFullPath($BackupRoot).TrimEnd([IO.Path]::DirectorySeparatorChar) + [IO.Path]::DirectorySeparatorChar
$ApplyEvidenceDirectory = (Resolve-Path -LiteralPath $ApplyEvidenceDirectory).Path
if (-not ($ApplyEvidenceDirectory + [IO.Path]::DirectorySeparatorChar).StartsWith($BackupRootResolved,[StringComparison]::OrdinalIgnoreCase)) {
  throw 'Apply evidence is outside the protected backup root.'
}
...
if ($PriorApplyText -notmatch 'Applying migration 20260719000003_company_reviews\.sql' -or $PriorApplyText -notmatch 'Finished supabase db push') {
  throw 'Prior apply output does not prove exact M4 completion.'
}
if ((@([regex]::Matches($PriorDryRunText,'\b\d{14}_[a-z0-9_]+\.sql\b') | ForEach-Object { $_.Value } | Sort-Object -Unique) -join ',') -ne $M4Filename) {
  throw 'Prior dry run was not exact M4.'
}
if (Test-Path -LiteralPath $PostDirectory) { throw 'Fresh post-M4 verification directory is not unused.' }

Push-Location $RepositoryRoot
try {
  if ((git branch --show-current).Trim() -ne $ExpectedBranch) { throw 'Branch changed before post-M4 verification.' }
  ...
  if ((Get-FileHash -Algorithm SHA256 -LiteralPath $M4Path).Hash.ToLowerInvariant() -ne $M4Sha256) { throw 'M4 hash differs.' }
  & node scripts/database/validate-migrations.mjs
  Assert-LastExitCode 'Repository validation failed before post-M4 verification.'

  $SecurePassword = Read-Host 'Enter the PRODUCTION Supabase database password for READ-ONLY M4 post-verification' -AsSecureString
...
"@) | ConvertFrom-Json
  if ($PostState.database -ne $DatabaseName -or $PostState.connected_user -ne 'postgres') { throw 'Post-M4 routed identity mismatch.' }
  if ($PostState.history_signature -ne $ExpectedHistorySignature -or $PostState.counts_signature -ne '202,177,0,0,0,0,0') { throw 'Post-M4 history, counts, or zero-row invariant failed.' }
  ...
  $AnonymousBehavior = Invoke-ReadOnlyScalar "SET LOCAL ROLE anon; SELECT count(*)::text FROM public.reviews;"
  $AuthenticatedBehavior = Invoke-ReadOnlyScalar "SET LOCAL ROLE authenticated; SET LOCAL request.jwt.claim.sub='00000000-0000-4000-8000-000000000001'; SELECT count(*)::text FROM public.reviews;"
  $ServiceBehavior = Invoke-ReadOnlyScalar "SET LOCAL ROLE service_role; RESET request.jwt.claim.sub; SELECT count(*)::text FROM public.reviews;"
  if ($AnonymousBehavior -ne '0' -or $AuthenticatedBehavior -ne '0' -or $ServiceBehavior -ne '0') { throw 'Post-M4 zero-review role behavior failed.' }
  Assert-ReadOnlyQueryFails "SET LOCAL ROLE anon; SELECT reviewer_user_id FROM public.reviews LIMIT 1;" 'Anonymous reviewer identity read unexpectedly succeeded.'
  Assert-ReadOnlyQueryFails "SET LOCAL ROLE authenticated; SELECT moderation_status FROM public.reviews LIMIT 1;" 'Authenticated moderation-state read unexpectedly succeeded.'
  ...
  if (Test-Path -LiteralPath (Join-Path $ApplyEvidenceDirectory 'm4-cli-work')) { Copy-Item -Recurse -LiteralPath (Join-Path $ApplyEvidenceDirectory 'm4-cli-work') -Destination $PostDirectory }
  [ordered]@{anonymous_public_read=$AnonymousBehavior;authenticated_public_read=$AuthenticatedBehavior;service_role_read=$ServiceBehavior;anonymous_reviewer_identity_read=$false;authenticated_moderation_state_read=$false;production_review_rows=0;production_admin_rows=0} | ConvertTo-Json | Set-Content -Encoding utf8 -LiteralPath (Join-Path $PostDirectory 'role-matrix-verification.json')
  ...
  Write-Output "M4 post-verification passed without reapplying. Evidence package: $PostName"
} catch {
  $Failure = $_
  try { @{success=$false;status='failed';production_changes_made=$true;apply_reinvoked=$false;verification_only=$true;error=$Failure.Exception.Message} | ConvertTo-Json | Set-Content -Encoding utf8 -LiteralPath (Join-Path $BackupRoot 'm4-post-verification-latest-status.json') } catch {}
  if (Test-Path -LiteralPath $PostDirectory) {
    try {
      @{success=$false;status='failed';production_changes_made=$true;baseline_ddl_executed=$false;first_admin_created=$false;review_rows_created=0;apply_reinvoked=$false;verification_only=$true;error=$Failure.Exception.Message} | ConvertTo-Json | Set-Content -Encoding utf8 -LiteralPath (Join-Path $PostDirectory 'status.json')
      Write-EvidenceChecksumManifest
    } catch {}
  }
  throw $Failure
